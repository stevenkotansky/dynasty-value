...
def run_league(league_name, recommend_adds_within_x_value_points, recommend_maybe, offline, show_visual, print_tables, mobile):
    ...
    # Convert players_to_drop and players_to_add to DataFrames if they are not already
    ...
    if players_to_drop_df.empty is False and players_to_add_df.empty is False:
        printout = printout+"You should consider making the following transactions (overall):"
        any_valid_overall_recommendations = False
        for i in range(len(players_to_drop_df)):
            drop_player = players_to_drop_df.iloc[i]
    ...
    if position_based_recommendations:
        ...
        for drop_player, recommendation in position_based_recommendations.items():
            ...
            if valid_add_candidates:
                ...
                for i, add_candidate, add_suggestion in valid_add_candidates:
                    printout=printout+f"    {i}) {add_suggestion}: {add_candidate['Player']}, {add_candidate['Position']}, {add_candidate['Team']}, {int(add_candidate['Age'])}yo (Trade Value {add_candidate['avg_value']}, VORP {add_candidate['VORP']}, Redraft {add_candidate['redraft_rank']})"
        
        if not any_valid_position_based_recommendations:
            if not recommend_maybe:
                printout=printout+"No definitive position-based recommendations for dropping or adding players based on trade value differences."
            else:
        ...
        if not recommend_maybe:
            ...
            printout=printout+"\nNo position-based recommendations for dropping or adding players."
    print(printout)
    return bottom_roster_players, top_free_agents, printout